...
namespace FanficMobileVersion.Repositories
{
    ...
    { 
        private static FavoriteService _ServiceClientInstance;
        public static FavoriteService ServiceClientInstance
        {
            get
            {
                if (_ServiceClientInstance == null)
                    _ServiceClientInstance = new FavoriteService();
        ...
        public async Task AddLike(int uId, int fId, string accessToken)
        {
            ...
            var response = await client.PostAsync("favorites/add/", content);
        ...
        public async Task DeleteLike(int uId, int fId, string accessToken)
        {
            ...
            //var response = await client.PostAsync("auth/login/", content);
            var response = await client.PostAsync("favorites/remove/", content);
            //using (var stream = await response.Content.ReadAsStreamAsync())
            var stream = await response.Content.ReadAsStringAsync();
            var jsoncontent = JsonConvert.DeserializeObject<LoginApiResponseModel>(stream);
            response.EnsureSuccessStatusCode();  
        ...
        public async Task<bool> CheckLike(int uId, int fId, string accessToken)
        {
            ...
            //var response = await client.PostAsync("auth/login/", content);
            var response = await client.PostAsync("favorites/isUserAddedFavorite/", content);
            //using (var stream = await response.Content.ReadAsStreamAsync())
            var stream = await response.Content.ReadAsStringAsync();
            //var jsoncontent = JsonConvert.DeserializeObject<LoginApiResponseModel>(stream);
            if (stream == "true")
